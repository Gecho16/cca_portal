...
if (isset($_POST["submitAddSection"])) {
	...
	if ($count > 0) {
		header("Location: {$baseUrl}admin/academics?table=sections&error=Subject Code Already <b>Exists</b>");
		exit();
	}
	...
	if (!$stmt) {
		header("Location: {$baseUrl}admin/academics?table=sections&error=Add Entry <b>Error</b>");
		exit();
	}
	...
	if (mysqli_stmt_affected_rows($stmt) > 0) {
		header("Location: {$baseUrl}admin/academics?table=sections&success=Added <b>ENTRY</b> successfully");
	} else {
		header("Location: {$baseUrl}admin/academics?table=sections&error=Add Entry <b>Error</b>");
	}
...
if (isset($_POST["submitEditSection"])) {
	...
	// Error preparing the statement
	if (!$stmt) {
		header("Location: {$baseUrl}admin/academics?table=sections&error=Update Entry <b>Error</b>");
		exit();
	}
	...
	// Check if any rows were affected
	if (mysqli_stmt_affected_rows($stmt) > 0) {
		header("Location: {$baseUrl}admin/academics?table=sections&success=Updated <b>ENTRY</b> Successfully");
	} else {
		header("Location: {$baseUrl}admin/academics?table=sections&error=No <b>ENTRY</b> Info Updated");
	}
...
if (isset($_GET["deleteSection"])) {
    ...
    if (!mysqli_stmt_execute($stmt)) {
        mysqli_stmt_close($stmt);
        mysqli_close($conn);
		header("Location: {$baseUrl}admin/academics?table=sections&error=Delete Entry <b>Error</b>");
        exit();
    }
    ...
    header("Location: {$baseUrl}admin/academics?table=sections&success=<b>ENTRY</b> Deleted Successfully");
    exit();
}