...
if (isset($_GET["selectYear"])) {
	...
	if (mysqli_num_rows($queryRecords) > 0) {
		while ($row = mysqli_fetch_assoc($queryRecords)) {
			...
			$actionBullet .= "	<a class='dropdown-item' href='course/edit?course=" . $id . "' title='edit'>
									Edit
								</a>";
			$actionBullet .= "	<button type='button' class='dropdown-item'' data-bs-toggle='modal' data-bs-target='#deleteModal' data-bs-name='" . $course_code . "' data-bs-href='../../assets/includes/admin/academics/course.inc.php?deleteCourse&id=" . $id . "' title='delete'>
									Delete
								</button>";
			$actionBullet .= "</div>";
			$actionBullet .= "</div>";
